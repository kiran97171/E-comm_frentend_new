...
const WiredHeadphones = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/wiredheadphones")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Wired Headphones</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 106 && item.id <= 110 }).map((element, index) => (
