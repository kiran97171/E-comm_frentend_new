...
const Earphones = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/earphones")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Earphones</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 111 && item.id <= 115 }).map((element, index) => (
