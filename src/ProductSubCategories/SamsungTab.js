...
const SamsungTab = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/samsungtab")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Samsung</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 161 && item.id <= 165 }).map((element, index) => (
