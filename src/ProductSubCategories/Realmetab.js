...
const Realmetab = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/realmetab")
    ...
    return (
        <>
            ...
            <h4 className='headings'>realme</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 176 && item.id <= 180 }).map((element, index) => (
