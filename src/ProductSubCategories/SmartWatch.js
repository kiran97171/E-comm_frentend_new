...
const SmartWatch = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/smartwatch")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Smart Watch</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 131 && item.id <= 135 }).map((element, index) => (
