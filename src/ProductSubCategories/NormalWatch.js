...
const NormalWatch = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/normalwatch")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Normal Watch</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 136 && item.id <= 140 }).map((element, index) => (
