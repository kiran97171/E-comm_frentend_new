...
const Bluetooth = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/bluetooth")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Wireless On-ear Bluetooth</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 116 && item.id <= 120 }).map((element, index) => (
