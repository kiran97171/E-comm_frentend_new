...
const Xiaomitab = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/productsub/xiaomitab")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Xiaomi</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 171 && item.id <= 175 }).map((element, index) => (
