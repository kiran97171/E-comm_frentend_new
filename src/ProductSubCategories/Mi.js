...
const Mi = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/mi")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Mi</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 51 && item.id <= 55 }).map((element, index) => (
