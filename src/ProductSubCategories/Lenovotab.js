...
const Lenovotab = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/lenovotab")
    ...
    return (
        <>
            ...
            <h4 className='headings'>Lenovo</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 166 && item.id <= 170 }).map((element, index) => (
