...
const HP = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/hp")
    ...
    return (
        <>
            ...
            <h4 className='headings'>HP</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 76 && item.id <= 80 }).map((element, index) => (
