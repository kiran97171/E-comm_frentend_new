...
const Headphone = ({ addToCart, count }) => {
    ...
    useEffect(() => {
        axios.get("https://e-comm-backend-new03.onrender.com/products/headphones")
        ...
        <>
            ...
            <div className='product-category-dropdown-container'>
                <div className="product-category-dropdown">
                    <button className="product-category-dropdown-btn">SHOP BY CATEGORY IN HEADEPHONES<IoIosArrowDown /></button>
                    <div className="product-category-dropdown-content">
                        <Link to='/headphone/wiredheadphones'>Wired Headphones</Link>
                        <Link to='/headphone/earphones'>Earphones</Link>
                        <Link to='/headphone/bluetooth'>Bluetooth</Link>
                        
                    </div>
                </div>
            </div>
            <h4 className='headings'>Headphone</h4>
            <div className='products-data-container'>
                <div className='products-data'>
                    {
                        data.slice(0, visible).filter((item, index) => { return item.id >= 91 && item.id <= 120 }).map((element, index) => (
